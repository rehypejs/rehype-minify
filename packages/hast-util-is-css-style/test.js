...
test('hast-util-is-css-style', function (t) {
  ...
  t.equal(ok(h('style', {type: 'text/css'})), true, 'yes - `style` with `[type=text/css]`');
  t.equal(ok(h('style', {type: 'TEXT/CSS'})), true, 'yes - `style` with `[type=TEXT/CSS]`');
  t.equal(ok(h('style', {type: 'TeXt/CsS'})), true, 'yes - `style` with `[type=TeXt/CsS]`');
  t.equal(ok(h('style', {type: ' text/css '})), true, 'yes - `style` with `[type= text/css ]`');
  t.equal(ok(u('element', {tagName: 'style'})), true, 'yes - without properties');

  t.equal(ok(h('style', {type: 'text/foo'})), false, 'no - `style` with `[type=text/foo]`');
